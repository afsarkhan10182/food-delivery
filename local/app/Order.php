...
class Order extends Authenticatable
{
   ...
   public function addNew($data)
   {
      ...
      $restaurant_user     = User::find($this->getStore($data['cart_no']));
   ...
   public function editOrder($data, $id)
   {
      ...
      $order->discount           = 0;
      $order->total              = 0;
      $order->commision_type     = User::orderBy('id', 'DESC')->first()->c_type;
      $order->commision_value    = User::orderBy('id', 'DESC')->first()->c_value;
      $order->balance_amount     = User::orderBy('id', 'DESC')->first()->balance_amount;
      $order->save();
   ...
   public function balanceAmount($balance_amount, $id)
   {
      $data['store_id'] = $id;
      $user = new User;
      $com = $user->getCom($id, $balance_amount);
      return $balance_amount - $com;
   }
}
