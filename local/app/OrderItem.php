...
class OrderItem extends Authenticatable
{
  ...
  public function editOrder($data, $id)
  {
    ...
    for ($i = 0; $i < count($item); $i++) {
      ...
      if ($unit[$i] == 1) {
        $price = $it->discount_kids_price ? $it->discount_kids_price : $it->kids_price;
      } elseif ($unit[$i] == 2) {
        $price = $it->discount_small_price ? $it->discount_small_price : $it->small_price;
      } elseif ($unit[$i] == 3) {
        $price = $it->discount_medium_price ? $it->discount_medium_price : $it->medium_price;
      } else {
        $price = $it->discount_large_price ? $it->discount_large_price : $it->large_price;
      }
